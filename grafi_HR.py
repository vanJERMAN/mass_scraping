...
def izdelava_grafov():
	...
	datum = date.today()
	wb = openpyxl.load_workbook(filename = f"{path}/csv_in_xlsx_datoteke_HR/{datum}.xlsx")
	ws = wb.worksheets[0] 
	sheet = wb.active
	xlsx_path = f"{path}/csv_in_xlsx_datoteke_HR/{datum}.xlsx"
	...
	for row in range(1, sheet.max_row):
		...
		for st in range(10, sheet.max_column):
			na_zalogi=0
			ni_na_zalogi=0
			inputWorkbook = xlrd.open_workbook(xlsx_path)
			...
			if na_zalogi > 0 and ni_na_zalogi > 0:	
				...
				datum = date.today()
				# print(datum)
				plt.savefig(f'{path}/slike_HR/grafi/{url_index}.{stevilka}.{datum}.png')
			...
			if st == sheet.max_column-1:
				...
				if celoten_artikel_na_zalogi == 0:
					...
					plt.tight_layout()
					datum = date.today()
					plt.savefig(f'{path}/slike_HR/grafi/{url_index}.vse_stevilke.{datum}.png')
			...
			if row == sheet.max_row-1:
				...
				plt.tight_layout()
				plt.legend(labels)
				plt.savefig(f'{path}/slike_HR/grafi/vsi_artikli.{datum}.png')
				print(f'vsi_artikli.{datum}.png')
				plt.clf()
				plt.cla()

	wb.save(filename = f"{path}/csv_in_xlsx_datoteke_HR/{datum}.xlsx")
	print(path_articles)
...
def izdelava_stolpcev():
	...
	for row in range(2, sheet.max_row):
		...
		path_xlsx = f"{path}/csv_in_xlsx_datoteke_HR/{datum}.xlsx"
		...
		plt.tight_layout()
		plt.savefig(f'{path}/slike_HR/grafi/zgodovina.{url_index}.{datum}.png')
	...
	plt.xticks(rotation=90)
	plt.tight_layout()
	plt.savefig(f'{path}/slike_HR/grafi/zgodovina_prodaje_vseh_artiklov.{datum}.png')
...
def vstavitev_grafov():
	...
	ws = wb.worksheets[0] 
	sheet = wb.active
	article_path = f"{path}/csv_in_xlsx_datoteke_HR/{datum}.xlsx"

	inputWorkbook = xlrd.open_workbook(article_path)
	inputWorksheet = inputWorkbook.sheet_by_index(0)
	datum = date.today()
	data = pd.read_excel(rf'{path}/csv_in_xlsx_datoteke_HR/{datum}.xlsx')
	img = openpyxl.drawing.image.Image(f"{path}/slike_HR/grafi/vsi_artikli.{datum}.png")	
	ws.add_image(img, f"B{sheet.max_row}")
	print(f"Celotna kolekcija graf je dodan")
	img = openpyxl.drawing.image.Image(f"{path}/slike_HR/grafi/zgodovina_prodaje_vseh_artiklov.{datum}.png")	
	...
	for row in range(1, sheet.max_row-1):
		...
		datum = date.today()
		data = pd.read_excel(rf'{path}/csv_in_xlsx_datoteke_HR/{datum}.xlsx')
		img = openpyxl.drawing.image.Image(f"{path}/slike_HR/grafi/{url_index}.vse_stevilke.{datum}.png")	
		ws.add_image(img, f"B{row+1}")
		print(f"row {row} je dodana slika")
		img = openpyxl.drawing.image.Image(f"{path}/slike_HR/grafi/zgodovina.{url_index}.{datum}.png")	
		...
		for st in range(10, sheet.max_column):
			...
			try:
				img = openpyxl.drawing.image.Image(f"{path}/slike_HR/grafi/{url_index}.{stevilka}.{datum}.png")
				if st == 10:
