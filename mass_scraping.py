...
import pandas as pd
from vstavitev_slik import vstavitev_slike
from zaloga import stevilke, zaloga
from grafi import izdelava_grafov, izdelava_stolpcev, vstavitev_grafov
...
def scraping(): 
	...
	pages = "https://www.mass.si/znamke/guess"
	datum = date.today()

	linki = []

	with open(f"{path}/csv_in_xlsx_datoteke/mass_data.csv", "w") as csv_file:
	...
	while pages:
		...
		with open(f"{path}/csv_in_xlsx_datoteke/mass_data.csv", "a") as csv_file:
			csv_writer = writer(csv_file)
			rand = randint(5, 10)
			...
			for article in articles:
				...
				try:
					procent_brez_znaka = round(100 - (((float(nova_cena[:5].replace(",", ".")) * 100) / float(stara_cena[:-2].replace(",", ".")))))
					procent = f"{procent_brez_znaka}%"
				except ValueError:
					procent = "-"

				file_path = f"{path}/slike/"
				url = article.find("img")["data-src"]
				filename = url[-22:]
				full_path = f"{file_path}{filename}"
				file_path_check = f"{path}/slike/{filename}"
		...
		# print(linki)
	read_file = pd.read_csv (rf"{path}/csv_in_xlsx_datoteke/mass_data.csv")
	read_file.to_excel (rf"{path}/csv_in_xlsx_datoteke/{datum}.xlsx", index = None, header=True)
	...
	while True:
		try:
			...
			try:
				...
				posiljanje("vanjermancek@gmail.com", "There was an error while running vstavitev_slike()", f"{error}", "no_attachment")
				break
		try:
			izdelava_grafov()
		except:
			try:
				...
				posiljanje("vanjermancek@gmail.com", "There was an error while running izdelava_grafov()", f"{error}", "no_attachment")
				break
		try:
			izdelava_stolpcev()
		except:
			try:
				...
				posiljanje("vanjermancek@gmail.com", "There was an error while running izdelava_stolpcev()", f"{error}", "no_attachment")
				break
		try:
			vstavitev_grafov()
		except:
		...
		try:
			...
			try:
				...
				posiljanje("vanjermancek@gmail.com", "There was an error while running posiljanje()", f"{error}", "no_attachment")
				break
