	...
	for row in range(1, sheet.max_row):
		...
		for st in range(10, sheet.max_column):
			...
			if na_zalogi > 0 and ni_na_zalogi > 0:	
				...
				datum = date.today()
				# print(datum)
				plt.savefig(f'{path}/slike/grafi/{url_index}.{stevilka}.{datum}.png')
			...
			if st == sheet.max_column-1:
				...
				if celoten_artikel_na_zalogi == 0:
					...
					plt.tight_layout()
					datum = date.today()
					plt.savefig(f'{path}/slike/grafi/{url_index}.vse_stevilke.{datum}.png')
			...
			if row == sheet.max_row-1:
				...
				plt.tight_layout()
				plt.legend(labels)
				plt.savefig(f'{path}/slike/grafi/vsi_artikli.{datum}.png')
...
def vstavitev_grafov():
	...
	ws = wb.worksheets[0] 
	sheet = wb.active
	article_path = f"{path}/csv_in_xlsx_datoteke/{datum}.xlsx"

	inputWorkbook = xlrd.open_workbook(article_path)
	inputWorksheet = inputWorkbook.sheet_by_index(0)
	datum = date.today()
	data = pd.read_excel(rf'{path}/csv_in_xlsx_datoteke/{datum}.xlsx')
	img = openpyxl.drawing.image.Image(f"{path}/slike/grafi/vsi_artikli.{datum}.png")	
	ws.add_image(img, f"B{sheet.max_row}")
	print(f"Celotna kolekcija graf je dodan")
	img = openpyxl.drawing.image.Image(f"{path}/slike/grafi/zgodovina_prodaje_vseh_artiklov.{datum}.png")	
	...
	for row in range(1, sheet.max_row-1):
		...
		datum = date.today()
		data = pd.read_excel(rf'{path}/csv_in_xlsx_datoteke/{datum}.xlsx')
		img = openpyxl.drawing.image.Image(f"{path}/slike/grafi/{url_index}.vse_stevilke.{datum}.png")	
		ws.add_image(img, f"B{row+1}")
		print(f"row {row} je dodana slika")
		img = openpyxl.drawing.image.Image(f"{path}/slike/grafi/zgodovina.{url_index}.{datum}.png")	
		...
		for st in range(10, sheet.max_column):
			...
			try:
				img = openpyxl.drawing.image.Image(f"{path}/slike/grafi/{url_index}.{stevilka}.{datum}.png")
				if st == 10:
